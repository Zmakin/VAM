    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 flex items-center justify-between p-4 border-b bg-white">
          <h2 className="text-lg font-semibold text-gray-900">Recurring Transfer Details</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                />
                <span className="font-semibold text-gray-900">{sourceAccount?.name || 'Unknown'}</span>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <span className="font-semibold text-gray-900">{targetAccount?.name || 'Unknown'}</span>
              </div>
            </div>
          ...
          <div>
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className={`font-medium ${allocation.isActive ? 'text-green-600' : 'text-gray-600'}`}>
                  {allocation.isActive ? 'Active' : 'Paused'}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Created:</span>
                <span className="font-medium text-gray-900">{formatDate(allocation.createdAt)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Start Date:</span>
                <span className="font-medium text-gray-900">{formatDate(allocation.startDate)}</span>
              </div>
              {allocation.endDate && (
                <div className="flex justify-between">
                  <span className="text-gray-600">End Date:</span>
                  <span className="font-medium text-gray-900">{formatDate(allocation.endDate)}</span>
                </div>
              )}
              {!allocation.endDate && (
                <div className="flex justify-between">
                  <span className="text-gray-600">Duration:</span>
                  <span className="font-medium text-gray-900">Indefinite</span>
                </div>
              )}
              {allocation.lastExecutedAt && (
                <div className="flex justify-between">
                  <span className="text-gray-600">Last Execution:</span>
                  <span className="font-medium text-gray-900">{formatDate(allocation.lastExecutedAt)}</span>
                </div>
              )}
              {nextExecution && (
                <div className="flex justify-between">
                  <span className="text-gray-600">Next Execution:</span>
                  <span className="font-medium text-green-600">{formatDate(nextExecution.toISOString())}</span>
                </div>
              )}
            </div>
          </div>
          ...
          {allocation.memo && (
            <div>
              <h3 className="text-sm font-semibold text-gray-900 mb-2">Memo</h3>
          ...
          {isScheduledTransaction && (
            <div>
              <h3 className="text-sm font-semibold text-gray-900 mb-2">This Transaction</h3>
              <div className="bg-gray-50 p-3 rounded-lg text-sm">
                <div className="flex justify-between mb-1">
                  <span className="text-gray-600">Executed On:</span>
                  <span className="font-medium text-gray-900">{formatDateTime(transaction.occurredAt)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Transaction ID:</span>
                  <span className="font-mono text-xs text-gray-600">{transaction.id.slice(0, 8)}...</span>
                </div>
              </div>
            </div>
          )}
              ...
              <div className="space-y-2">
                {isScheduledTransaction && onDeleteTransaction && (
                  <button
                    ...
                    className="w-full px-4 py-3 text-left border-2 border-gray-200 rounded-lg hover:border-red-300 hover:bg-red-50 transition-all"
                  >
                    <div className="font-medium text-gray-900">Delete This Transaction Only</div>
                ...
                <button
                  ...
                  className="w-full px-4 py-3 text-left border-2 border-gray-200 rounded-lg hover:border-red-300 hover:bg-red-50 transition-all"
                >
                  <div className="font-medium text-gray-900">Delete Entire Series</div>
                  <div className="text-xs text-gray-600 mt-1">
            ...
            <div className="border-2 border-red-200 bg-red-50 rounded-lg p-4">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 py-2 px-4 border border-gray-300 text-gray-700 rounded-lg hover:bg-white font-medium"
                >
                  Cancel
                </button>
                <button
                  onClick={showDeleteConfirm === 'single' ? handleDeleteSingle : handleDeleteSeries}
                  className="flex-1 py-2 px-4 bg-red-600 text-white rounded-lg hover:bg-red-700 font-medium flex items-center justify-center gap-2"
                >
                  <Trash2 size={16} />
                  Delete
                </button>
